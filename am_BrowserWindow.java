...
/**
 * This package-private class defines one am_browser window for use with
 ...
 */
class am_BrowserWindow extends Stage {

    private am_WebBrowser am_owner; // the WebBrowser application
    private WebEngine am_webEngine; // loads and manages pages
    private Menu am_windowMenu; // holds window-related commands

    /**
     * The constructor sets up the window, and starts loading the
     * initial web page location, if given. It does not show the window.
     * 
     * @param am_browser    the WebBrowser application that is createing this window
     * @param am_initialURL if non-null, the window will attempt to load this URL
     */
    am_BrowserWindow(am_WebBrowser am_browser, String am_initialURL) {
        ...
        am_webEngine = webview.getEngine();
        ...
        am_webEngine.locationProperty().addListener((o, oldVal, newVal) -> {
        ...
        am_webEngine.titleProperty().addListener((o, oldVal, newVal) -> {
            if (newVal == null)
                setTitle("Untitled " + am_owner.am_getNextUntitledCount());
            else
                setTitle(newVal);
        });
        ...
        am_webEngine.getLoadWorker().stateProperty().addListener((o, oldVal, newVal) -> {
        ...
        cancelButton.setOnAction(e -> { // Cancel the ongoing page load
            if (am_webEngine.getLoadWorker().getState() == Worker.State.RUNNING)
                am_webEngine.getLoadWorker().cancel();
        ...
        am_webEngine.setOnAlert(evt -> SimpleDialogs.message(evt.getData(), "Alert from web page"));
        am_webEngine.setPromptHandler(promptData -> SimpleDialogs.prompt(promptData.getMessage(),
                "Query from web page", promptData.getDefaultValue()));
        am_webEngine.setConfirmHandler(str -> SimpleDialogs.confirm(str, "Confirmation Needed").equals("yes"));

        /* If the am_initialURL is not null, load that page! */

        if (am_initialURL != null) {
            // load the initial web page, if any
            doLoad(am_initialURL);
        }
    ...
    private MenuBar makeMenuBar() {
        ...
        newWin.setOnAction(e -> am_owner.am_newBrowserWindow(null));
        ...
        open.setOnAction(e -> {
            ...
            if (url != null && url.trim().length() > 0)
                am_owner.am_newBrowserWindow(url);
        });
        am_windowMenu = new Menu("Window");
        am_windowMenu.getItems().addAll(newWin, close, open, new SeparatorMenuItem());
        am_windowMenu.setOnShowing(e -> populateWindowMenu());
        MenuBar menubar = new MenuBar(am_windowMenu);
        return menubar;
    }
    ...
    private void populateWindowMenu() {
        ArrayList<am_BrowserWindow> windows = am_owner.getOpenWindowList();
        while (am_windowMenu.getItems().size() > 4) {
            ...
            am_windowMenu.getItems().remove(am_windowMenu.getItems().size() - 1);
        }
        if (windows.size() > 1) {
            ...
            am_windowMenu.getItems().add(item);
            am_windowMenu.getItems().add(new SeparatorMenuItem());
        }
        for (am_BrowserWindow window : windows) {
            ...
            final am_BrowserWindow win = window; // (for use in a lambda expression)
